...
async fn get_json_end_to_end_with_caching() {
    ...
    let users: JsonFetcher<(&str, u64), User> =
        JsonFetcher::get(reqwest::Client::new(), move |(_, id)| {
...
async fn non_2xx_surfaces_as_a_status_error() {
    ...
    let users: JsonFetcher<u64, User> = JsonFetcher::get(reqwest::Client::new(), move |id| {
        format!("{base}/users/{id}")
    });
...
#[tokio::test]
async fn request_builder_controls_method_and_headers() {
    ...
    let items: JsonFetcher<u64, User> =
        JsonFetcher::request(reqwest::Client::new(), move |http, id| {
            http.get(format!("{base}/items?id={id}"))
                .header("x-swr", "on")
        });
